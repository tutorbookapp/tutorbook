...
class Timeslot implements TimeslotInterface {
  ...
  toString(includeDay: boolean = false) {
    let str = this.from.toLocaleTimeString() + ' - ' + 
    ...
    if (includeDay) {
      if (this.from.getDay() === this.to.getDay()) {
        ...
        str = DAYS[this.from.getDay()] + ' ' + str.split(' - ')[0] + ' - ' + 
          DAYS[this.to.getDay()] + ' ' + str.split(' - ')[1];
      }
    }
    return str;
  }
  ...
  equalTo(timeslot: TimeslotInterface): boolean {
    return timeslot.from.valueOf() === this.from.valueOf() && 
...
export default class ScheduleInput extends React.Component<ScheduleInputProps> {
   ...
   */
  static times: Readonly<Timeslot[]> = [new Timeslot(
  ...
  static get timeslots(): Availability {
    ...
    for (let day = 0; day < 7; day++) 
      for (const time of ScheduleInput.times) timeslots.push(new Timeslot(
        ...
        >
          <DataTable>
            <DataTableContent>
              ...
              <DataTableBody>
                {ScheduleInput.times.map((timeslot: Timeslot, index: number) => (
                  <DataTableRow key={index}>
                    ...
                    {DAYS.map((day: string, index: number) => {
                      const timeslotCheckboxRepresents = new Timeslot( 
                        ScheduleInput.getNextDateWithDay(index as Day, timeslot.from),
                        ScheduleInput.getNextDateWithDay(index as Day, timeslot.to),
                      );
                      return (
                        <DataTableCell key={day} hasFormControl>
                          <Checkbox 
                            checked={this.isChecked(timeslotCheckboxRepresents)} 
                            onChange={evt => {
                              this.setChecked(timeslotCheckboxRepresents, evt);
                            }}
                          />
                        </DataTableCell>
                      );

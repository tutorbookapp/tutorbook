...
export function decode(params: Record<string, string>): UsersQuery {
  ...
  if (params.parents) query.parents = json(params.parents);
  if (params.orgs) query.orgs = json(params.orgs);
  if (params.tags) query.tags = json(params.tags);
  if (params.aspect) query.aspect = Aspect.parse(params.aspect);
  if (params.langs) query.langs = json(params.langs);
  if (params.subjects) query.subjects = json(params.subjects);
  if (params.availability) query.availability = json(params.availability);
